        ...
        for item in sensor_dict:
            if item.get('EventName') == "Stop Arm ST":
                ...
                while True:
                    ...
                    cv2.line(blank, (0, 250), (350, 250), (0, 0, 255), 2)
                    cv2.line(blank, (0, 400), (350, 400), (0, 0, 255), 2)

                    # Draw lines on bottom-right
                    # ((1340, 735), (1920, 735)) ~ values
                    bottom_right_upper_zone = (np.subtract(np.multiply(3, np.floor_divide(blank.shape[1], 4)), 100),
                                               np.subtract(np.multiply(3, np.floor_divide(blank.shape[0], 4)), 75)),\
                                              (blank.shape[1], np.subtract(np.multiply(3, np.floor_divide(blank.shape[0], 4)), 75))
                    bottom_right_lower_zone = (np.subtract(np.multiply(3, np.floor_divide(blank.shape[1], 4)), 100),
                                               np.add(np.multiply(3, np.floor_divide(blank.shape[0], 4)), 120)),\
                                              (blank.shape[1], np.add(np.multiply(3, np.floor_divide(blank.shape[0], 4)), 120))
                    # backup zone variables
                    bruz = (int(3 * blank.shape[1] // 4) - 100, int(3 * blank.shape[0] // 4) - 75),\
                           (blank.shape[1], int(3 * blank.shape[0] // 4)-75)
                    brlz = (int(3 * blank.shape[1] // 4) - 100, int(3 * blank.shape[0] // 4) + 120),\
                           (blank.shape[1], int(3 * blank.shape[0] // 4) + 120)
                    # print('bottom_right_upper_zone:', bottom_right_upper_zone)
                    # print('bottom_right_lower_zone:', bottom_right_lower_zone)

                    # screen visuals
                    cv2.line(blank, (int(3 * blank.shape[1] // 4), int(3 * blank.shape[0] // 4) + 25),
                             (blank.shape[1], int(3 * blank.shape[0] // 4) + 25), (255, 0, 0), 5)
                    cv2.line(blank, (int(3 * blank.shape[1] // 4), int(3 * blank.shape[0] // 4) + 200),
                             (blank.shape[1], int(3 * blank.shape[0] // 4) + 200), (255, 0, 0), 5)
                    if x1 <= 350:
                        # print('hi')
                        if 250 <= y1 <= 400:
                            try:
